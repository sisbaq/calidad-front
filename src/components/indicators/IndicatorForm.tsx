...
const extractFormulaSuggestions = (formula: string): string[] => {
  ...
  const nonNumeric = rawChunks.filter(
    (chunk) => !/^[0-9.,]+$/.test(chunk),
  );
...
const buildInitialValues = (
  ...
  return {
    ...
    estimatedValue: String(initial.estimatedValue),
    variables:
...
export const IndicatorForm: FC<IndicatorFormProps> = ({
  ...
  const handleTextChange =
    (field: keyof IndicatorFormValues) =>
  ...
  const handlePeriodicityChange = (event: SelectChangeEvent) => {
    setValues((prev) => ({
      ...prev,
      periodicity: event.target.value as Indicator['periodicity'],
    }));
  };
  ...
  const handleTrendChange = (event: SelectChangeEvent) => {
    setValues((prev) => ({
      ...prev,
      trend: event.target.value as Indicator['trend'],
    }));
  };

  const handleVariableFieldChange =
    (id: string, field: keyof IndicatorVariable) =>
      (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const estimatedValueStr = values.estimatedValue.trim();
    ...
    if (!estimatedValueStr) {
      setError('El valor estimado es obligatorio.');
      return;
    }

    const annualTarget = Number(annualTargetStr);
    const realValue = Number(realValueStr);
    const estimatedValue = Number(estimatedValueStr);
    ...
    if (!Number.isFinite(realValue)) {
      setError('El valor real debe ser un número válido.');
      return;
    }
    ...
    if (cleanedVariables.length === 0) {
      setError(
        'Define al menos una variable con Nombre de la variable y Descripción.',
      );
      return;
    }
    ...
    if (expectedNames.length === 0) {
      setError(
        'La fórmula debe contener al menos una variable (no solo números u operadores).',
      );
      return;
    }
    ...
    const missingVariablesFromFormula = expectedNames.filter(
      (seg) =>
        !cleanedVariables.some((v) => toLower(v.key) === toLower(seg)),
    );

    const extraVariablesNotInFormula = cleanedVariables.filter(
      (v) =>
        !expectedNames.some((seg) => toLower(seg) === toLower(v.key)),
    );

    if (missingVariablesFromFormula.length > 0) {
      const list = missingVariablesFromFormula
        .map((s) => `"${s}"`)
        .join(', ');
      setError(
        `Debes definir una variable para cada nombre usado en la fórmula. Faltan variables para: ${list}.`,
      );
      return;
    }

    if (extraVariablesNotInFormula.length > 0) {
      const list = extraVariablesNotInFormula
        .map((v) => `"${v.key}"`)
        .join(', ');
      setError(
        `Algunas variables definidas no aparecen en la fórmula: ${list}. Revisa los nombres o ajusta la fórmula.`,
      );
      return;
    }
  ...
  return (
    ...
    >
      <Typography
        variant="h6"
        sx={{ fontWeight: 600, mb: 1, color: appColors.blue }}
      >
        {isEditing ? 'Editar Indicador' : 'Crear Nuevo Indicador'}
      </Typography>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Define la información básica, la fórmula y la configuración de tu
        indicador.
      </Typography>
      ...
      <Box component="form" noValidate autoComplete="off" onSubmit={handleSubmit}>
                  ...
                  InputProps={{
                    endAdornment: (
                      <InputAdornment position="end">
                        <Tooltip
                          title={
                            'Escribe la fórmula usando los mismos nombres de variable que registrarás debajo. Ejemplo: riesgos controlados / riesgos totales * 100'
                          }
                        >
                          <InfoOutlinedIcon
                            fontSize="small"
                            sx={{ color: appColors.blue, cursor: 'pointer' }}
                          />
                        </Tooltip>
                      </InputAdornment>
                    ),
        ...
        <Box sx={{ mt: 2 }}>
          <Typography
            variant="subtitle2"
            sx={{ fontWeight: 600, color: appColors.green, mb: 1 }}
          >
          ...
          >
            <Typography
              ...
              sx={{ mr: 2, flex: 1 }}
            >
              Primero escribe la fórmula del indicador. Luego, en{' '}
              <Box component="span" fontWeight={600}>
                Nombre de la variable
              </Box>{' '}
              ingresa o selecciona desde la ayuda los textos que utilizaste en la fórmula
              (por ejemplo: "riesgos controlados" y "riesgos totales"). En{' '}
              <Box component="span" fontWeight={600}>
                Descripción de la variable
              </Box>{' '}
              explica claramente qué representa cada una.
            </Typography>
          ...
          <TableContainer
            ...
            sx={{
              boxShadow: 'none',
              border: `1px solid ${appColors.blue}1A`,
            }}
          >
                  ...
                  <TableRow key={variable.id}>
                      ...
                      <Autocomplete
                        ...
                        renderInput={(params) => (
                          <TextField
                            ...
                            fullWidth
                            size="small"
                            placeholder="Ej: riesgos controlados"
                          />
                        )}
                      />
                    </TableCell>
                    <TableCell>
                      <TextField
                        ...
                        required
                        size="small"
                        placeholder="Ej: Número de riesgos controlados"
            ...
            <Grid container spacing={2}>
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField
                  ...
                  label="Meta Anual"
                  type="number"
                  value={values.annualTarget}
                  onChange={handleTextChange('annualTarget')}
                  size="small"
              ...
              <Grid size={{ xs: 12, md: 2 }}>
                <TextField
                  ...
                  onChange={handleTextChange('realValue')}
                  size="small"
              ...
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField
                  ...
                  onChange={handleTextChange('estimatedValue')}
                  size="small"
                  inputProps={{ step: '0.01' }}
                />
              </Grid>
